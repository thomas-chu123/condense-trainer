...
class Condenser(nn.Module):
    def __init__(
        ...
        self.norm = nn.LayerNorm(self.hidden_size * self.n_last_hidden_states).to(
        ...
        self.linear = nn.Linear(
            self.hidden_size * self.n_last_hidden_states,
            self.base_model_hidden_size,
            bias=True,
    ...
    @torch.no_grad()
    def forward(
        self, context_ids, prompt_ids=None, attention_mask=None
    ...
    def _condense_context(self, context_ids, attention_mask) -> torch.Tensor:
        ...
        expaned_attention_mask = torch.cat(
            [
                attention_mask,
                torch.ones(
        ...
        output = self.condense_model(
            ...
            attention_mask=expaned_attention_mask,
        )
    ...
    def generate(self, context: str, prompt: str, **kwargs):
        ...
        condensed_tokens, inputs_embeds = self.forward(
            context_ids, prompt_ids, attention_mask
        )
...
if __name__ == "__main__":
    ...
    context = dataset[0]["context"][:4000]
    ...
    condense_model = AutoModelForCausalLM.from_pretrained(
        condense_model_id, torch_dtype=torch.bfloat16
    ).to("cuda")
    condense_tokenizer = AutoTokenizer.from_pretrained(condense_base_model_id)
    decoder_model = AutoModelForCausalLM.from_pretrained(
        decoder_model_id, torch_dtype=torch.bfloat16
    ).to("cuda")
    decoder_tokenizer = AutoTokenizer.from_pretrained(decoder_model_id)

    file_path = huggingface_hub.hf_hub_download(
        repo_id=condense_model_id, filename="checkpoints/modules.pt", local_dir="./"
    )
